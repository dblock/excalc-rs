...
- **Input modes**: one-shot CLI argument, piped stdin, or an interactive REPL (`excalc`/`calc` with no expression while stdin is a terminal) — the REPL has line history and Tab completion of function names (via `rustyline`) and `help`/`about`/`vars`/`exit`/`quit` commands.
