...
- Degree/radian support: `deg(x)`/`rad(x)` convert between radians and degrees, and `sind`/`cosd`/`tand`/`asind`/`acosd`/`atand` are degree-native variants of the six basic circular trig functions. Deliberately stateless/explicit rather than reviving the original's global `CalcMode` toggle — see `docs/functions/standard-math.md` and `DESIGN.md#angle-units`.
