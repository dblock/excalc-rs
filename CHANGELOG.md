...
- User-defined functions: `name(params) := expr` defines a function (e.g. `f(x) := x^2 + 1`), visible to later statements the same way a variable assignment is, listed alongside variables by the REPL's `vars` command. Functions can call themselves or each other; arguments are evaluated in the caller's scope (proper lexical scoping, no accidental self-shadowing). Since the language has no `if`/branching construct, self-recursive calls can't stop at a computed base case; there's no fixed call-count limit either, but each nested call checks actual remaining stack space and fails gracefully with `function call recursion limit exceeded` once it's running low, rather than crashing with a native stack overflow. Function names can't collide with built-ins or `pi`/`e`, and definitions are session-only, matching variable persistence.
