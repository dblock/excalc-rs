...
- User-defined functions: `name(params) := expr` defines a function (e.g. `f(x) := x^2 + 1`), visible to later statements the same way a variable assignment is, listed alongside variables by the REPL's `vars` command (showing the actual definition, e.g. `f(x) = x ^ 2 + 1`, not a placeholder). Functions can call themselves or each other; arguments are evaluated in the caller's scope (proper lexical scoping, no accidental self-shadowing). There's no fixed call-count limit; each nested call checks actual remaining stack space and fails gracefully with `function call recursion limit exceeded` once it's running low, rather than crashing with a native stack overflow. Function names can't collide with built-ins or `pi`/`e`, and definitions are session-only, matching variable persistence.
- Conditionals: `cond ? then : else` (ternary operator) and its exact equivalent `if(cond, then, else)` — both short-circuit, evaluating only the taken branch, which lets self-recursive user-defined functions actually terminate at a computed base case instead of always recursing, e.g. `fact(n) := n < 2 ? 1 : n * fact(n - 1)`.
- Digit-group separators: number literals may use `,` or `_` as a thousands separator (e.g. `1,234` or `1_234_567`); a group must be exactly 3 digits, so short argument lists like `sum(1,2)` still parse as 2 arguments, not a single grouped number. Whichever separator is used first in an expression is echoed back in the printed result, e.g. `1,000 + 1` prints `1,001`.
- Currency symbols: number literals may be prefixed with `$`, `£`, `€`, or `¥` (e.g. `$100`), which is purely cosmetic and ignored for math. Whichever currency symbol is used first in an expression is echoed back in the printed result, e.g. `$1 + 10` prints `$11`; math across different currency symbols isn't tracked or rejected.
- REPL command history now persists across sessions, saved to `~/.excalc_history` by default (`%USERPROFILE%\.excalc_history` on Windows if `$HOME` isn't set); override the location with `--history-file <path>` or the `EXCALC_HISTORY_FILE` environment variable, or disable persistence entirely with `--no-history`.
