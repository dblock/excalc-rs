...
- Interactive REPL: running `calc`/`excalc` with no expression while stdin is a terminal now starts a read-eval-print loop (variables persist across lines within the session), with `help` for usage, `about` for version/author/license info, `vars` to list assigned variables, and `exit`/`quit`/Ctrl-D to leave, instead of just printing usage.
