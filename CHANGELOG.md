...
- Advanced / special functions: `gamma`, `beta`, `pochhammer`, `bth`, `bman`, `ellipticE`, `ellipticF` (alias `ellipticK`), `ellipticCE`, `ellipticCK`, `dilog`, `dawson`, `erf`, `erfc`, `si`, `ssi`, `ci`, `chi`, `fresnelC`, `fresnelS`, `fresnelF`, `fresnelG`. Most of these are thin wrappers around a generic numeric-integration primitive in the original engine, so this port evaluates them with a private adaptive Simpson's-rule integrator as a stand-in; see `docs/functions/advanced.md` for a TODO on routing them through the general-purpose numeric integration engine below.
